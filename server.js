...
// A simple middleware to allow all origins just for this test.
app.use((req, res, next) => {
  res.setHeader('Access-Control-Allow-Origin', '*');
  next();
});

// This is our main diagnostic route. It's a GET request.
app.get('/api/contact/', (req, res) => {
  // We log to the Vercel console to be sure.
  console.log('DIAGNOSTIC TEST: API route has been hit.');

  const apiKey = process.env.RESEND_API_KEY;

  if (apiKey && apiKey.length > 10) {
    // If the key exists and looks like a real key...
    console.log('DIAGNOSTIC TEST: API Key IS PRESENT.');
    // We send a success message back to the browser.
    res.status(200).json({
      status: 'SUCCESS',
      message: 'The RESEND_API_KEY is present and loaded in the Vercel environment.',
      key_status: `Key is set and ends with ...${apiKey.slice(-4)}`
    });
  } else {
    // If the key is missing or empty...
    console.log('DIAGNOSTIC TEST: API Key is MISSING or UNDEFINED.');
    // We send an error message back to the browser.
    res.status(500).json({
      status: 'FAILURE',
      message: 'The RESEND_API_KEY is MISSING or UNDEFINED in the Vercel environment.'
    });
  }
});
